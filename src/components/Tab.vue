...
export default {
  ...
  created() {
    this.$store.dispatch('acSetSidebar', this.menuList[0].title);
  },
  computed: {
  ...
  methods: {
    scrollTo(i, e) {
      ...
      // 加载二级列表
      this.$store.dispatch('acSetSidebar', this.menuList[i].title);
      window.console.log(this.menuList[i].title);
      // window.console.log(this.sidebarList[0]);
      this.$store.dispatch('acRequestGoodList');
      setTimeout(() => {
        this.$store.dispatch('acGetGoodsList', { type: this.sidebarList[0], page: 1, size: 5 });
      }, 500);
    },
  },
};
